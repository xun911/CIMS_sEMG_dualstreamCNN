...
import time
import os
from .symbol_multistream_dynetwork import get_symbol
...
#        shapes.update({'gesture_softmax_label': (batch_size, 52)})        
...
class Module(LSTMMixin, AdaBNMixin, FaugMixin, Base):
    ...
    def __init__(self, **kargs):
        ...
        self.multi_stream = kargs.pop('multi_stream', False)
        self.num_stream = kargs.pop('num_stream', 1)
    ...
    def predict(self, eval_data, *args, **kargs):
        ...
        if not self.binded:
            self.bind(data_shapes=[('stream%d_data' % i, (eval_data.batch_size, self.num_channel[i], 
                                                          self.num_semg_row[i], self.num_semg_col[i])) for i in range(len(self.num_semg_row))],
                      for_training=False)
