...
class CrossValEvaluation(Evaluation):
    ...
    def vote_accuracy_curves_processprob(self, exps, folds, windows, feature_name, window, num_semg_row, num_semg_col, balance=False):
        acc = []
        for exp in exps:
            for fold in folds:
                acc.append(self.vote_accuracy_curve_processprob(
                    Mod=exp.Mod,
                    dataset=exp.dataset,
                    fold=int(fold),
                    windows=windows,
    ...
    def vote_accuracy_curve_processprob(self, Mod, dataset, fold, windows, feature_name, window, num_semg_row, num_semg_col,
                            dataset_args=None,
                            balance=False):
        get_crossval_val = self.get_crossval_val_func(dataset)
        pred_proba, true, segment = _crossval_predict(
            self,
            Mod=Mod,
            get_crossval_val=get_crossval_val,
            fold=fold,
            ...
            num_semg_col = num_semg_col,
            dataset_args=dataset_args)
        from .vote_processprob import get_vote_accuracy_curve as do
        return do(true, pred_proba, segment, windows, balance)[1]
