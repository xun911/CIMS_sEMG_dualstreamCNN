...
class CapgMyoOverlapPieceWiseMultiStreamData(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CapgMyoOverlapPieceWiseMultiStreamData, self).__init__(*args, **kargs)

        self.piece_width = [3, 4, 4, 4, 4, 4, 4, 3]
           
        self.num_channel = []
        self.num_stripe = 8
        self.shapes1 = []        
        self.shapes2 = []             
        for width in self.piece_width:
            self.num_channel.append(self.window)
            self.shapes1.append(width)
...
#        print data.shape
        start_end = [[0,3],[1,5],[3,7],[5,9],[7,11],[9,13],[11,15],[13,16]]
        res = []
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
    ...
    def reset(self):
        self._reset()
        super(CapgMyoOverlapPieceWiseMultiStreamData, self).reset()
