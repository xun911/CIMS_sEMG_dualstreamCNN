...
class CapgMyoPieceWiseMultiStreamData_V2(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CapgMyoPieceWiseMultiStreamData_V2, self).__init__(*args, **kargs)

        self.piece_width = 2
           
        self.num_channel = []
        self.num_stripe = int(self.num_semg_row/self.piece_width)
        self.shapes1 = []        
        self.shapes2 = []             
        for i in range(self.num_stripe):
            self.num_channel.append(self.window)
            self.shapes1.append(self.piece_width)
            self.shapes2.append(int(self.num_semg_col))
...
#        print data.shape
        res = []
        for i in range(self.num_stripe):
              start = i*self.piece_width
              end = start+self.piece_width
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
    ...
    def reset(self):
        self._reset()
        super(CapgMyoPieceWiseMultiStreamData_V2, self).reset()
