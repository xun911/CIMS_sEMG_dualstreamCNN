...
class CapgMyoRawimgFFTMultiStreamData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(CapgMyoRawimgFFTMultiStreamData, self).__init__(*args, **kargs)
...
#        print data.shape
        ...
        for amp in Context.parallel(jb.delayed(_get_fft_aux)(sample) for sample in data):
            res.append(amp)
...
#              frame_data = data[i,:,:,:]
#              ch_num = data.shape[1]
#              frame_data = frame_data.reshape(self.num_semg_row, self.num_semg_col)              
#              frame_data = np.fft.fft2(frame_data)
#              frame_data = np.fft.fftshift(frame_data)
#              frame_data = 20*np.log(np.abs(frame_data))
##              print frame_data.shape
##              print ch_num
#              frame_data = frame_data.reshape(1, ch_num, frame_data.shape[0], frame_data.shape[1])
...
#             print res[0].shape    
             ...
             res2 = self._get_fft(res2.asnumpy() if isinstance(res2, mx.nd.NDArray) else res2)     
    ...
    def reset(self):
        self._reset()
        super(CapgMyoRawimgFFTMultiStreamData, self).reset()
...
def _get_fft_aux(frame_data):
         frame_data = frame_data.reshape(16, 8)              
         frame_data = np.fft.fft2(frame_data)
         frame_data = np.fft.fftshift(frame_data)
#         frame_data = 20*np.log(np.abs(frame_data))
         frame_data = frame_data.reshape(1, 1, frame_data.shape[0], frame_data.shape[1])  
         return frame_data