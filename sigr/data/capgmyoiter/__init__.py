...
from piecewise_multistream_iter_overlap import CapgMyoOverlapPieceWiseMultiStreamData
...
from rawimg_fftimg_multistream_iter import CapgMyoRawimgFFTMultiStreamData
...
class Dataset(Base):
    ...
    root = ('/home/weiwentao/public/duyu/misc/dba')
...
#    trials = list(range(10))
    ...
    def parse(cls, text):
        if text == 'capgmyoiter':
           return cls(root = '/home/weiwentao/public/duyu/misc/dba')
...
#            index.append(np.arange(n, n + len(seg)))
        ...
        elif feature_name == 'rawimg_fftimg_multistream':
             return CapgMyoRawimgFFTMultiStreamData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
        ...
        elif feature_name == 'piece_multistream_overlap':
             return CapgMyoOverlapPieceWiseMultiStreamData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
