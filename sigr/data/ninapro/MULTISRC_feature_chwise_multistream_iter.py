...
class ChwiseFeatureMultiStreamIter_v2(mx.io.NDArrayIter):
...
#        self.feature_list = kargs.pop('feature_list')
        ...
        super(ChwiseFeatureMultiStreamIter_v2, self).__init__(*args, **kargs)
...
#        self.shape2 = self.num_semg_col*self.num_semg_row
        ...
        for i in range(total_ch):
            ...
            self.shapes.append(32)
            
        for i in range(total_ch):
            self.num_channel.append(1)
            self.shapes.append(22)    
            
        for i in range(total_ch):
            self.num_channel.append(1)
            self.shapes.append(28)        
...
#        print self.shape1
    ...
    def provide_data(self):
         ...
         res = [('stream%d_' % i + self.data[0][0], tuple([self.batch_size, ch] + list([1, self.shapes[i]])))
...
#        print data_2.shape
    ...
    def _getdata(self, data_source):
        ...
        if data_source is self.data:       
             ...
             res = new_res 
#             print res[0].shape                         
             res = [a.reshape(a.shape[0], a.shape[2], -1) for a in res]             
             assert (len(res) == 1)   
             kk = res[0]
    ...
    def reset(self):
        self._reset()
        super(ChwiseFeatureMultiStreamIter_v2, self).reset()
