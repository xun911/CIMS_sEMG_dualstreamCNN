...
class RawDataFeatureImageSingleStreamIter(mx.io.NDArrayIter):
...
#        self.feature_list = kargs.pop('feature_list')
        ...
        super(RawDataFeatureImageSingleStreamIter, self).__init__(*args, **kargs)
...
#         res = [('stream%d_data' % i, tuple([self.batch_size, ch] + list([self.shape1, self.num_semg_col*self.num_semg_row])))
#                   for i, ch in enumerate(self.num_channel)]   
...
#             res = np.concatenate((res[0],res[1]),axis=2)
##             print 'data concat!!!!!!!!!!!!!'             
...
#        res = new_res
    ...
    def reset(self):
        self._reset()
        super(RawDataFeatureImageSingleStreamIter, self).reset()
