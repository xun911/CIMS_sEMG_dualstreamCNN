...
#    feature_extraction_winstride = constant.FEATURE_EXTRACTION_WIN_STRIDE   
    ...
    semg_root = '/data/datasets/ninapro-db2-var-raw/data' 
#    semg_root = None


    feature_root = ('/data/ninapro-feature/ninapro-db2-downsample20-var-raw-prepro-lowpass-win-%d-stride-%d/' % (feature_extraction_winlength, 
...
#        )
    ...
    def get_dataiter(self, get_trial, combos, feature_name, window=1, adabn=False, mean=None, scale=1000000.0, **kargs):
...
#        data = np.vstack(data).reshape(-1, 1, self.num_semg_row, self.num_semg_col)
        ...
        fea_dims = data.shape[1]
        semg_dims = data.shape[2]
       
        data = data.reshape(data.shape[0], fea_dims*semg_dims)
...
class GetTrial(object):
    ...
    def __call__(self, semg_root, feature_root, combo):
        ...
        if semg_root is None:
            ...
            if len(semg_data) == len(semg_feature):
                pass
            elif len(semg_data) < len(semg_feature):
                data_length = len(semg_data)
                print ('use first %d samples of semg features for data length balance' % data_length)
                semg_feature = semg_feature[:data_length,:,:]
                assert (len(semg_data) == len(semg_feature)) 
                
            elif len(semg_data) > len(semg_feature):    
                data_length = len(semg_feature)
                print ('use first %d samples of semg data for data length balance' % data_length)
                semg_data = semg_data[:data_length,:,:]
                assert (len(semg_data) == len(semg_feature)) 
