...
from block_multistream_iter import BlockMultiStreamData
from block_multistream_iter_v2 import BlockMultiStreamData_v2
from ch_multistream_plus_rawimg_iter import  ChMultiStreamPlusRawImgData
...
from subwin_multistream_iter import SubWindowMultiStreamData
from subwin_multistream_iter_v2 import SubWindowMultiStreamData_v2
from simplestacked_iter import SimpleStackedData
from single_frame_multistream_iter import SingleFrameMultiStreamData
...
#    feature_extraction_winstride = constant.FEATURE_EXTRACTION_WIN_STRIDE   
    ...
    root = ('/home/weiwentao/data/ninapro-db1/data')
...
#            index.append(np.arange(n, n + len(seg)))
        ...
        if feature_name == 'actimg':  
        ...
        elif feature_name == 'featureimage': 
              feature_list = constant.FEATURE_LIST
        ...
        elif feature_name == 'chdiffimage':
             return ChDiffImageData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
        ...
        elif feature_name == 'chdiffsigimage':
             return ChDiffSigImageData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
        ...
        elif feature_name == 'chdiff_sigimg_multistream': 
            return ChDiffSigImgMultiStreamData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
        ...
        elif feature_name == 'featureimg_rawimg_multistream':   
            feature_list = constant.FEATURE_LIST
            return FeatureRawImgMultiStreamData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
        ...
        elif feature_name == 'ch_multistream':
            return ChMultiStreamData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
            ...
            return BlockMultiStreamData_v2(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
        ...
        elif feature_name == 'ch_multistream_plus_rawimg':
            return ChMultiStreamPlusRawImgData(
                        data=OrderedDict([('data', data)]),
                        label=OrderedDict(label),
                        gesture=gesture.copy(),
                        subject=subject.copy(),
        ...
        elif feature_name == 'sigimg_rawimg_multistream':
            return SigImgRawImgMultiStreamData(
                                        data=OrderedDict([('data', data)]),
                                        label=OrderedDict(label),
                                        gesture=gesture.copy(),
                                        subject=subject.copy(),
        ...
        elif feature_name == 'frame_multistream':
            return FrameMultiStreamData(
                                        data=OrderedDict([('data', data)]),
                                        label=OrderedDict(label),
                                        gesture=gesture.copy(),
                                        subject=subject.copy(),
        ...
        elif feature_name == 'subwin_multistream':
            return SubWindowMultiStreamData(
                                        data=OrderedDict([('data', data)]),
                                        label=OrderedDict(label),
                                        gesture=gesture.copy(),
                                        subject=subject.copy(),
        ...
        elif feature_name == 'subwin_multistream_v2':
            return SubWindowMultiStreamData_v2(
                                        data=OrderedDict([('data', data)]),
                                        label=OrderedDict(label),
                                        gesture=gesture.copy(),
                                        subject=subject.copy(),
        ...
        elif feature_name == 'simplestackedimg':
            return SimpleStackedData(
                                        data=OrderedDict([('data', data)]),
                                        label=OrderedDict(label),
                                        gesture=gesture.copy(),
                                        subject=subject.copy(),
             ...
             return SingleFrameMultiStreamData(
                                        data=OrderedDict([('data', data)]),
                                        label=OrderedDict(label),
                                        gesture=gesture.copy(),
                                        subject=subject.copy(),
...
#         res = [(k, tuple([self.batch_size, self.num_channel] + list(v.shape[2:]))) for k, v in self.data]
    ...
    def _get_actimg_1d(self, data):
        
        from ... import Context
        import joblib as jb
        res = []
         
        for amp in Context.parallel(jb.delayed(_get_actimg_1d_aux)(sample) for sample in data):
...
#        assert self.window > 1        
        ...
        if data_source is self.data:       
             ...
             if self.window == 1:
                  res = [self._get_actimg_1d(a.asnumpy() if isinstance(a, mx.nd.NDArray) else a) for a in res]
             else:
                  res = [self._get_actimg(a.asnumpy() if isinstance(a, mx.nd.NDArray) else a) for a in res]
#             print res[0].shape             
...
class SignalImageData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        self.num_semg_col = kargs.pop('num_semg_col', 0)
        self.num_semg_row = kargs.pop('num_semg_row', 0)
        super(SignalImageData, self).__init__(*args, **kargs)

       
        self.num_data = len(self._index)
        self.data_orig = self.data
        self.reset()
#        print self._subject.shape
...
#         res = [(k, tuple([self.batch_size, self.num_channel] + list(v.shape[2:]))) for k, v in self.data]
    ...
    def reset(self):
        self._reset()
        super(SignalImageData, self).reset()
...
class RawImageData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        self.num_semg_col = kargs.pop('num_semg_col', 0)
        self.num_semg_row = kargs.pop('num_semg_row', 0)
        super(RawImageData, self).__init__(*args, **kargs)
...
def _get_actimg_aux(data):
    ...
    return np.transpose(dft(get_sig_img(data.T)))  
    
def _get_actimg_1d_aux(data):
      
    return np.transpose(dft_1d_hy(get_sig_img(data.T)))    
    
def _get_sigimg_aux(data):    
            
    return np.transpose(get_sig_img(data.T))   
    
#def _get_feature_aux(data, feature_list):
#    
#    return np.transpose(get_feature(data.T, feature_list)) 
#
...
#            single_channel.append(eval(str(func))(data[i,:]))
