...
class FrameMultiStreamData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(FrameMultiStreamData, self).__init__(*args, **kargs)
...
#        self.block_width = int(num_block_stripes*2)
        ...
        self.total_ch = self.window - self.win_num + 1
#        self.num_stripe = int(self.num_semg_row / 2 - num_block_stripes + 1)
        ...
        for i in range(self.total_ch):
            ...
            self.shapes2.append(self.win_num)
...
#        print data.shape
        ...
        for i in range(self.total_ch):
              start = i
              end = start+self.win_num 
              frame_data = data[:,start:end,:]
              frame_data = frame_data.reshape(frame_data.shape[0], 1, -1, end-start)
    ...
    def reset(self):
        self._reset()
        super(FrameMultiStreamData, self).reset()
