...
class ChwiseRawIMGFeatureMultiStreamIter_v2(mx.io.NDArrayIter):
...
#        self.feature_list = kargs.pop('feature_list')
        ...
        super(ChwiseRawIMGFeatureMultiStreamIter_v2, self).__init__(*args, **kargs)
        ...
        self.num_channel = []
        self.shapes = []        
        for i in range(self.data[0][1].shape[3]):
            self.num_channel.append(1)
            self.shapes.append(self.shape1+self.shape2)
        
#        for i in range(self.data[1][1].shape[3]):
#            self.num_channel.append(1)
...
#         assert_equal(len(self.data), 1)
         res = [('stream%d_data' % i, tuple([self.batch_size, ch] + list([1, self.shapes[i]])))
    ...
    def _get_ch(self, data_source):

        

#        print data.shape
        ch_num = data_source[0].shape[2]
        for i in range(len(data_source)):
            assert (data_source[i].shape[2] == ch_num)
...
#        return res      
        ...
        for i in range(ch_num):
            ...
            for j in range(len(data_source)):
              ...
              ch_data = data[:,:,i]
              ch_data = ch_data.reshape(ch_data.shape[0], 1, -1, ch_data.shape[1])
              ch_res.append(ch_data)
            
            ch_res = np.concatenate(ch_res, axis = 3)      
#            print ch_res.shape
            res.append(ch_res)              
        return res      
...
#             kk = res[0]
    ...
    def reset(self):
        self._reset()
        super(ChwiseRawIMGFeatureMultiStreamIter_v2, self).reset()
